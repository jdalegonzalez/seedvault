...
### Product specification split (added after round-2 external review)

The reviewers' sharpest structural point: a lessons-learned narrative isn't a buildable spec until it separates what is Anna Bell's configuration from what is SeedVault360's product. The split, for Dale:

**Anna Bell configuration (one tenant's settings, not product):** mac-and-cheese SKUs and their noodle-ounce conversion factors; the five-pound batch target; PRFM disposition types (frozen-stickered, boxed-at-sale, walk-in, tasting); HRA's $6 barter-discounted price vs. the $7 standard; Venture Milk and the specific vendor list; the current workbook cell mappings. All of this lives in config tables, never in code.

**Reusable SV360 product (the platform):** a normalized customer model separating brand, location/property, contact/buyer, ordering party, bill-to/payer, distributor, and per-SKU price agreements with effective dates — the flattened spreadsheet version at Anna Bell already shows why conflating these breaks (a "Hyatt Seattle" row and a "US Foods" row can double-count the same order); order-intake normalization with a read-only approval loop (extract candidates from email/text/PO with source message ID, timestamp, match confidence, and duplicate detection; founder approves; only approved records become demand); a batch/yield engine (integer-batch optimization with configurable padding priority); an inventory-event ledger (planned inbound, delivered outbound, market transfers); receivables reconciliation (invoice → payment ID → processing fee → net deposit → bank match, with commitments recorded as collection notes, never payment status); three-tier revenue classification (committed/invoiced, event targets vs. actuals, speculative inventory value — never rolled into one number, a trap our own first Excel version fell into and round-2 review caught); cash-gap calculation with named cost exclusions until rates are configured; and the underwriting evidence package assembled from all of the above. Barter terms (HRA's free kitchen against a $1/pan discount) must be a first-class, underwritable component — non-cash consideration is common in this industry and invisible to every P&L.

**Customer-zero proof metrics (what Anna Bell must demonstrate to make this sellable):** order-entry time reduced; production variance (planned vs. actual batches); inventory variance and stockouts/overproduction avoided; overdue AR collected (baseline: $504 aged receivable surfaced on day one of connector access); days from order to cleared cash; weekly cash-forecast accuracy; gross margin by channel; founder hours removed per week. We'll instrument these as the weekly runs accumulate — they become the sales deck.
